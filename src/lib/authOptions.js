        ...
        async jwt({ token, user }) {
            try {
                if (user?.email) {
                    ...
                    // Ensure default admin setup if this email is part of DEFAULT_ADMIN_EMAILS
                    try {
                        const { ensureAdminSetup } = await import('@/lib/db');
                        await ensureAdminSetup(user.email);
                    } catch (e) {
                        console.warn('ensureAdminSetup failed during sign-in:', e?.message || e);
                    }
